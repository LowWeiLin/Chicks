    ...
    public Vector3 getSteering(ICollection<Rigidbody> targets)
    {
        ...
        foreach (Rigidbody r in targets)
        {
            ...
            if (dist < maxSepDist)
            {
                ...
                var strength = sepMaxAcceleration * (maxSepDist - dist) / Mathf.Pow(maxSepDist - boundingRadius - targetRadius, 1);

				// TODO: fix this bug
				if ((maxSepDist - boundingRadius - targetRadius) == 0) {
					Debug.Log ("DIV BY ZERO");
					strength = 0;
				}
